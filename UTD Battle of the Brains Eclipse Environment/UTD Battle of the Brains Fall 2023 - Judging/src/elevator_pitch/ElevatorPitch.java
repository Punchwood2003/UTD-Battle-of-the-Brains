...
public class ElevatorPitch {
	...
	private class Elevator {
		...
		private HashMap<Integer, HashMap<Request, Long>> unfulfilledRequests;
		private HashMap<Integer, HashMap<Request, Long>> processingRequests;
		...
		public Elevator() {
			this.unfulfilledRequests = new HashMap<Integer, HashMap<Request, Long>>();
			this.processingRequests = new HashMap<Integer, HashMap<Request, Long>>();
		...
		public void addRequest(Request request) {
			...
			HashMap<Request, Long> floor = (this.unfulfilledRequests.containsKey(request.startingFloor))
					? this.unfulfilledRequests.get(request.startingFloor)
							: new HashMap<Request, Long>();
			if(floor.containsKey(request)) {
				floor.put(request, floor.get(request)+1);
			} else {
				floor.put(request, 1L);
			}
		...
		private long exit() {
			...
			if(this.processingRequests.containsKey(this.currentFloor)) {
				HashMap<Request, Long> floor = this.processingRequests.get(this.currentFloor);
		...
		private long board() {
			...
			if(this.unfulfilledRequests.containsKey(this.currentFloor)) {
				...
				HashMap<Request, Long> floor = this.unfulfilledRequests.get(this.currentFloor);
				for(Request request : floor.keySet()) {
					DirOutput dirs = this.getRelativeDirection(request);
					if(dirs.destinationFloorDir == this.direction) {
						...
						HashMap<Request, Long> destinationFloor = (containsKey) ? 
								this.processingRequests.get(request.destinationFloor) : new HashMap<Request, Long>();
		...
		public TurnOutput handleTurn(PrintWriter out, long currTurn, boolean isDoneWithInput) {
			long numExited = this.exit();
			this.determineDirection();
			long numBoarded = this.board();
