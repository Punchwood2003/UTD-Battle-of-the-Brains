...
const (
	...
	NUM_TEST_CASES                 = 16
	MAX_N                          = 10000                // n <= 100
	MAX_PER_LINE                   = 10                   // The total number of requests will be no more than 10
	MAX_CD                         = int(^uint32(0) >> 1) // c, d <= 2^{31}-1
	NONE_CHANCE                    = 0.3                  // 30% chance of none
)
...
func main() {
	...
	for testCaseNumber = END_OF_HAND_CRAFTED_TEST_CASES; testCaseNumber < NUM_TEST_CASES; testCaseNumber++ {
		...
		n := rand.Intn(ratio) + (ratio * (testCaseNumber - 1))
		fmt.Fprintf(inputWriter, "%d\n", n)
		for i := 0; i < n; i++ {
			// Generate the number of requests
			chance := rand.Float32()
			if chance <= NONE_CHANCE {
				fmt.Fprintf(inputWriter, "None\n")
			} else {
				numRequests := rand.Intn(MAX_PER_LINE) + 1
				for j := 0; j < numRequests; j++ {
					// Generate the starting and ending floors
					startingFloor := rand.Intn(MAX_CD)
					endingFloor := rand.Intn(MAX_CD)
					for startingFloor == endingFloor {
						endingFloor = rand.Intn(MAX_CD)
					}
					fmt.Fprintf(inputWriter, "%d->%d", startingFloor, endingFloor)
					if j != numRequests-1 {
						fmt.Fprintf(inputWriter, ",")
					}
				}
				fmt.Fprintf(inputWriter, "\n")
			}
